...
from cnn_inference import evaluate
from utils import board_to_tensor, create_board_history
...
ACTIONS = [] # (dx, dy, is_promotion, promotion_piece)
...
def move_to_index(move: chess.Move) -> int:
    # from_square and to_square are zero based
...
class ChessAI:
    def __init__(self, model, color=chess.WHITE, sims=100, c_puct=1.0, device=None):
        self.model = model
        self.color = color
        self.sims = sims
        self.c_puct = c_puct
    ...
    def select_move(self, board: chess.Board, use_dirichlet=False, temperature=0.0) -> chess.Move:
        ...
        # Build history everytime select_move is called
        game_history = create_board_history(board)

        # Initial evaluation
        v_root, policy_root = self._evaluate(canon, list(game_history))
        ...
        for _ in range(self.sims):
            ...
            self._simulate(root, board_copy, game_history)
    ...
    def _simulate(self, root, board, game_history):
        ...
        if board.is_game_over():
            ...
            leaf_history = game_history.copy()
            leaf_history.popleft()
            leaf_history.append(board.copy())
            v, policy = self._evaluate(board, list(leaf_history))
            node.P = policy
    ...
    def _evaluate(self, board: chess.Board, history):
        ...
        # 🛠️ Make sure tensor is on the same device as the model (cuda or cpu)
        input = board_to_tensor(history)



        policy, v = evaluate(self.model, self.device, input)

        # Clamp value prediction to [-1.0, 1.0]
        v = max(-1.0, min(1.0, v))
    ...
    #     v, policy = evaluate(self.model, self.device, input)
...
def main():
    board = chess.Board()
    ai = ChessAI(CNN(), color=chess.WHITE, sims=1, c_puct=1.5)
    # ai.select_move(board, use_dirichlet=False, temperature=0.0)
    i = 0
    while not board.is_game_over() or i < 5:
        print(board)
        i+=1
